...
export default function TasksPage() {
  ...
  const goToPrevPage = () => {
    if (pagination.hasPreviousPage) goToPage(currentPage - 1);
  };
  ...
  return (
    <section className="space-y-4">
      ...
      {/* Pagination Controls & Tasks Info */}
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 mt-1">
                    ...
                    <span className="text-xs text-slate-500">Created: {new Date(t.createdAt).toLocaleDateString()}</span>
                    {t.assignedTo && (
                      <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded">
                        Assigned to: {t.assignedTo.profile?.fullName || t.assignedTo.email}
                      </span>
                    )}
                    <span className={`text-xs px-2 py-1 rounded ${t.done ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Form method="post" style={{ display: 'inline-block' }}>
                    ...
                    <button
                      type="submit"
                      onClick={(e) => {
                        if (!confirm(`Are you sure you want to delete "${t.title}"?`)) e.preventDefault();
                      }}
                      style={{
                        padding: '4px 8px',
